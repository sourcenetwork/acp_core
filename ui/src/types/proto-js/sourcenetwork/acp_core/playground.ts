...
import type { SandboxData, SandboxDataErrors, SandboxRecord, SandboxTemplate } from "./sandbox";
import type { AnnotatedPolicyTheoremResult } from "./theorem";

export const protobufPackage = "sourcenetwork.acp_core";

export enum SerializationEncoding {
  UNKNOWN_ENCODING = 0,
  BASE64 = 1,
  BYTES = 2,
  UNRECOGNIZED = -1,
...
export interface SetSerializedStateRequest {
  /** handle to an existing sandbox */
  handle: number;
...
export interface SetSerializedStateResponse {
  /** ok indicates the state updated sucessfuly */
  ok: boolean;
  /** errors represent all recoverable errors found while parsing and setting the sandbox state */
  errors:
...
export interface GetSerializedStateRequest {
  /** handle to an existing sandbox */
  handle: number;
  encoding: SerializationEncoding;
}

export interface GetSerializedStateResponse {
  data: Uint8Array;
}
