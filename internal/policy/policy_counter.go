...
func newPolicyCounter(manager runtime.RuntimeManager) policyCounter {
...
func (r *policyCounter) GetNext(ctx context.Context) (uint64, error) {
	kv := r.getStore()
...
func (r *policyCounter) setCounter(ctx context.Context, counter uint64) error {
	kv := r.getStore()
...
func (r *policyCounter) Increment(ctx context.Context) error {
	_, err := r.GetNextAndIncrement(ctx)
	return err
}

// GetNextAndIncrement atomically gets the next free counter and increments it
func (r *policyCounter) GetNextAndIncrement(ctx context.Context) (uint64, error) {
