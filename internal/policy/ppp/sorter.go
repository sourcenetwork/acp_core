...
func (t *SortTransformer) Transform(pol types.Policy) (specification.TransformerResult, error) {
	...
	mgmtPermExtractor := func(permission *types.ManagementRule) string { return permission.Name }
	...
	for _, resource := range pol.Resources {
		...
		utils.FromExtractor(resource.ManagementRules, mgmtPermExtractor).SortInPlace()
	}
